...
financial_schema = StructType([
    ...
    StructField("car_company", StringType(), True)
])
...
def process_batch(batch_df, batch_id):
    ...
    operational_df = batch_df.select(
        ...
        coalesce(col("after.email"), col("before.email")).alias("email"),
        coalesce(col("after.gender"), col("before.gender")).alias("gender"),
        coalesce(col("after.ip_address"), col("before.ip_address")).alias("ip_address"),
        coalesce(col("after.car_company"), col("before.car_company")).alias("car_company"),
    ...
    # Perform DML MERGE
    spark.sql("""
    MERGE INTO iceberg.info_schema.information_tbl AS target
    USING (
        SELECT 
            id, 
            first_name, 
        ...
        UPDATE SET
            ...
            target.ip_address = s.ip_address,
            target.car_company = s.car_company
        WHEN NOT MATCHED AND s.op = 'c'
        THEN INSERT  (id,first_name, last_name, email, gender, ip_address, car_company) 
        VALUES (s.id, s.first_name, s.last_name, s.email, s.gender, s.ip_address, s.car_company  )        
        """)
...
def main():
    spark = SparkSession.builder \
        .appName("Postgres-CDC-to-Iceberg") \
    ...
    kafka_df = spark.readStream \
        ...
        .option("subscribe", "psq-perfix.info.information_tbl") \
    ...
    query = parsed_df.writeStream \
        ...
        .option("checkpointLocation", "s3a://bronze/checkpoints/information_cdc") \
